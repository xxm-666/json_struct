...
TEST(TypeConversion_NumericConversions) {
    ...
    JsonValue maxLongLong(std::numeric_limits<int64_t>::max());
    JsonValue minLongLong(std::numeric_limits<int64_t>::min());
    
    // 验证整数转换
    ASSERT_EQ(maxInt.toInt(), std::numeric_limits<int32_t>::max());
    ASSERT_EQ(minInt.toInt(), std::numeric_limits<int32_t>::min());
    ASSERT_EQ(maxLongLong.toLongLong(), std::numeric_limits<int64_t>::max());
    ASSERT_EQ(minLongLong.toLongLong(), std::numeric_limits<int64_t>::min());
...
TEST(TypeConversion_SafeIntegerRange) {
    // 测试IEEE 754安全整数范围
    int64_t safeMax = 9007199254740992LL;   // 2^53
    int64_t safeMin = -9007199254740992LL;  // -2^53
    int64_t unsafeMax = 9007199254740993LL; // 2^53 + 1
