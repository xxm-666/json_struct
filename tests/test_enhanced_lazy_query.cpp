...
TEST(EnhancedLazyRecursiveDescent) {
    ...
    while (gen.hasNext()) {
        ...
        if (result.value->isNumber()) {
            prices.push_back(result.value->toDouble());
        }
    }
...
TEST(EnhancedLazyResetFunctionality) {
    ...
    for (size_t i = 0; i < firstRun.size(); ++i) {
        ASSERT_EQ(firstRun[i], secondRun[i]);
    }
}
...
TEST(EnhancedLazyFilterFunction) {
    ...
    auto filterFunc = [](const JsonValue& value, const std::string& path) -> bool {
        return value.isNumber() && value.toDouble() > 15.0;
    };
    ...
    while (gen.hasNext()) {
        auto result = gen.next();
        if (result.value->isNumber()) {
            highValues.push_back(result.value->toDouble());
        }
    }
...
TEST(EnhancedLazyPerformanceComparison) {
    ...
    // Additional performance tests with different queries
    std::vector<std::string> performanceQueries = {
        "$.store.book[0].title",           // Single element access
        "$.store.book[2:5].price",         // Slice access  
        "$..price",                        // Recursive descent
        "$.store.book[?(@.price < 20)]"    // Filter expression
    };
    
    for (const auto& query : performanceQueries) {
        // Enhanced version
        timer.start();
        auto enhancedGen2 = EnhancedQueryFactory::createGenerator(filter, data, query);
        int enhancedCount = 0;
        while (enhancedGen2.hasNext()) {
            enhancedGen2.next();
            enhancedCount++;
        }
        long long enhancedTime2 = timer.end();
        
        // Traditional version
        timer.start();
        auto traditionalResults2 = filter.query(data, query);
        long long traditionalTime2 = timer.end();
        
        ASSERT_EQ(traditionalResults2.size(), enhancedCount);
        
        std::cout << "Query '" << query << "': Enhanced " << enhancedTime2 
                  << "μs vs Traditional " << traditionalTime2 << "μs" << std::endl;
    }
}

TEST(EnhancedLazyEarlyTermination) {
    JsonFilter filter = JsonFilter::createDefault();
    JsonValue data = createEnhancedTestData();
    
    PerformanceTimer timer;
    
    // Test early termination with maxResults
    timer.start();
    auto gen = EnhancedQueryFactory::createGenerator(filter, data, "$.store.book[*].title");
    std::vector<std::string> results;
    int maxResults = 5;
    int count = 0;
    while (gen.hasNext() && count < maxResults) {
        results.push_back(gen.next().value->toString());
        count++;
    }
    long long earlyTerminationTime = timer.end();
    
    // Test full traversal
    timer.start();
    auto fullResults = filter.query(data, "$.store.book[*].title");
    long long fullTraversalTime = timer.end();
    
    ASSERT_EQ(maxResults, results.size());
    ASSERT_TRUE(fullResults.size() > maxResults);
    
    std::cout << "Early termination (" << maxResults << " results): " << earlyTerminationTime 
              << "μs vs Full traversal (" << fullResults.size() << " results): " 
              << fullTraversalTime << "μs" << std::endl;
    
    // Early termination should be significantly faster for partial results
    std::cout << "Performance improvement: " << 
                 (static_cast<double>(fullTraversalTime) / earlyTerminationTime) << "x faster" << std::endl;
}

TEST(EnhancedLazyPerformanceStats) {
    JsonFilter filter = JsonFilter::createDefault();
    JsonValue data = createEnhancedTestData();
    ...
    while (gen.hasNext() && processedCount < 5) {
        gen.next();
        processedCount++;
    }
    
    // Get performance statistics
    std::string stats = gen.getPerformanceStats();
    std::cout << "Performance Statistics:\n" << stats << std::endl;
    
    // Verify stats contain expected information
    ASSERT_TRUE(stats.find("Frames Processed:") != std::string::npos);
    ASSERT_TRUE(stats.find("Results Generated:") != std::string::npos);
    ASSERT_TRUE(stats.find("Efficiency Ratio:") != std::string::npos);
}

TEST(EnhancedLazyCacheFunctionality) {
    JsonFilter filter = JsonFilter::createDefault();
    JsonValue data = createEnhancedTestData();
    ...
    ASSERT_TRUE(gen.getCacheSize() == 0);  // Empty initially
    
    // Process some results
    std::vector<JsonValue> results1;
    while (gen.hasNext() && results1.size() < 5) {
        results1.push_back(gen.next().value);
    }
    
    // Reset and process again (should use cache for some operations)
    gen.reset();
    std::vector<JsonValue> results2;
    while (gen.hasNext() && results2.size() < 5) {
    ...
    gen.enableCache(false);
    ASSERT_EQ(gen.getCacheSize(), 0u);
    
    gen.enableCache(true);
    ASSERT_EQ(gen.getCacheSize(), 0u);  // Should be cleared
    
    // Test cache hit ratio (should be 0 since we just cleared)
