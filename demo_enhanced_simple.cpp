...
void demonstrateBasics() {
    ...
    JsonValueEnhanced null_val;
    JsonValueEnhanced bool_val(true);
    JsonValueEnhanced int_val(42);
    JsonValueEnhanced str_val("Hello, World!");
    
    std::vector<int> numbers = {1, 2, 3, 4, 5};
    JsonValueEnhanced array_val(numbers);
    
    std::map<std::string, std::string> person = {
        {"name", "Alice"},
        {"city", "Beijing"}
    };
    JsonValueEnhanced object_val(person);
...
void demonstrateErrorHandling() {
    ...
    try {
        auto parsed = JsonValueEnhanced::parse(invalid_json);
    ...
    JsonValueEnhanced::ParseOptions depth_options;
    depth_options.maxDepth = 3;
    
    std::string deep_json = "[[[[42]]]]";
    
    try {
        auto parsed = JsonValueEnhanced::parse(deep_json, depth_options);
    } catch (const std::exception& e) {
...
int main() {
    std::cout << "JsonValueEnhanced Feature Demonstration" << std::endl;
    std::cout << "=======================================" << std::endl;
